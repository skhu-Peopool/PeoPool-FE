...
import { Send, MoreVertical, Smile, Paperclip, Search, Archive, Star, MessageCircle, Plus, AlertCircle } from "lucide-react";
...
const ChatInterface = () => {
        ...
        try {
          ...
          setTimeout(() => {
            setDirectChats(prevChats => {
              const targetChat = prevChats.find(chat => 
              ...
              if (targetChat) {
                ...
                console.log("타겟 채팅방을 찾을 수 없음. 사용 가능한 채팅방:", prevChats);
              }
    ...
    try {
      ...
      const formattedChats = response.chatRoomList.map(room => {
        ...
        return {
          ...
          receiverId: room.receiverId || room.opponentId || room.userId || room.memberId
        };
      });
      ...
      // TeamFinder에서 온 경우가 아니라면 첫 번째 채팅방을 자동 선택
      if (formattedChats.length > 0 && !location?.state?.justCreated) {
        const firstChat = formattedChats[0];
        console.log("첫 번째 채팅방 선택:", firstChat);
  ...
  const loadChatMessages = async (roomId) => {
    try {
      ...
      setError("메시지를 불러오는데 실패했습니다: " + (error.message || "알 수 없는 오류"));
      setMessages([]);
    }
  };
  ...
  const markRoomAsReadSafely = async (roomId) => {
    try {
      ...
      if (error.status === 404) {
        ...
        console.warn("HTTP 메소드가 허용되지 않음. PATCH 대신 POST나 PUT 시도 필요");
      }
    }
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // 실제 API 호출
      const response = await chatService.sendMessage(selectedChat.receiverId, messageToSend);
      ...
      setMessages(prev => prev.map(msg => 
        msg.id === tempMessage.id 
          ? {
              ...
              time: response.createdAt ? formatTime(response.createdAt) : tempMessage.time,
              isMe: response.isMine !== undefined ? response.isMine : true,
              avatar: "나",
      ...
      if (!selectedChat.receiverId) {
        errorMessage = "수신자 정보가 없습니다. 페이지를 새로고침하고 다시 시도해주세요.";
        ...
        {selectedChat ? (
            ...
            <div className="h-16 bg-white border-b border-gray-200 px-6 flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3">
                    <p className={`text-sm font-medium ${selectedChat.online ? 'text-green-600' : 'text-gray-500'}`}>
                      {selectedChat.online ? '온라인' : `마지막 접속: ${selectedChat.time}`}
                    </p>
                        ...
                        >
                          ...
                          {msg.sending && (
                            <div className="flex items-center gap-1 mt-1">
                              ...
                              <span className="text-xs opacity-60">전송 중...</span>
                            </div>
                          )}
                        </div>
          ...
          <div className="flex-1 flex items-center justify-center bg-white">
            <div className="text-center text-gray-500">
              ...
              <p className="text-sm">왼쪽에서 대화를 시작할 사람을 선택해보세요</p>
            </div>
          </div>
        )}
      </div>
